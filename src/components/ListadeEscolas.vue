  ...
  <div class="d-flex flex-column w-100">
    <v-row>
      <v-col cols="12" sm="6" md="4" lg="3" class="pt-8 pb-8 pl-8 d-flex align-center justify-center">
        ...
        <v-sheet height="200" width="300" elevation="2" rounded="lg" class="pa-4 d-flex flex-column align-center borda-diagonal 
      ...
      <v-col v-if="isLoading" v-for="n in 3" :key="n" cols="12" sm="6" md="4" lg="3" class="pa-8">
        <v-skeleton-loader class="mx-auto border" elevation="2" max-width="300"
          type="card, divider"></v-skeleton-loader>
      </v-col>

      <!-- Iteração das escolas existentes quando carregadas -->
      <v-col v-if="!isLoading" v-for="(escola, index) in escolas" :key="index" cols="12" sm="6" md="4" lg="3"
        class="pa-8">
        <v-sheet elevation="2" height="200" width="300" rounded="lg"
          class="pa-4 d-flex flex-column align-center borda-diagonal degrade clickable-item position-relative justify-center"
          @click="fetchTurmas(index, escola.id_escola)" style="cursor: pointer">
          <!-- Loader circular quando o card específico está em carregamento -->
          <div v-if="isCardLoading[index]" class="d-flex align-center justify-center degrade borda-diagonal"
